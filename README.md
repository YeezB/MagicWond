...
训练教程：
[EdgeImpulse环境配置](doc/edgeimpulseConfigTutorial.md)
[EdgeImpulse训练教程](doc/magicWondDataForward.md)

训练过程
- 如果后续想要新增手势，需要登录EdgeImpulse网站并且打开项目。
- 在魔法棒K10上上传[ModelTrain](src/ModelTrain/)代码，并且打开串口监视器，确保串口输出有加速度传感器数据
- 配置edgeimpulse数据上传环境（参考[EdgeImpulse环境配置](doc/edgeimpulseConfigTutorial.md)文档）
- 选择数据上传到magic-xhl这个项目（参考[EdgeImpulse训练教程](doc/magicWondDataForward.md)）
